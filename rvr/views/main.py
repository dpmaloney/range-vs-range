...
def is_what_now():
    ...
    """
    return "first_action" in request.cookies
...
def login():
    ...
    nxt = urllib.parse.urlparse(req)
    cur = urllib.parse.urlparse(request.url)
...
def home_page():
    """
    Generates the authenticated landing page. AKA the main page or home page.
    """
...
def _range_action_to_vars(item):
    ...
    return {"screenname": item.user.screenname,
            "fold_pct": old_div(100.0 * fold_total, total),
            "passive_pct": old_div(100.0 * passive_total, total),
            "aggressive_pct": old_div(100.0 * aggressive_total, total),
...
def _action_summary_to_vars(range_action, action_result, action_result_order,
    ...
    return {"screenname": user_range.user.screenname,
            ...
            "percent": old_div(100.0 * new_total, len(SET_ANYTHING_OPTIONS)),
            "combos": new_total,
